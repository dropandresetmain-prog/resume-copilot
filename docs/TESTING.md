...
| Cover letter | `cover-letter.test.ts`, `cover-letter-quality.test.ts`, `cover-letter-application-package.test.ts`, `cover-letter-pdf-preview.test.ts` |
...
| Application shell | `application-records.test.ts`, `application-package-ux.test.ts`, `workflow-paper-cuts.test.ts`, `jd.test.ts`, `files.test.ts`, `supabase.test.ts`, `enrichment.test.ts` |
...
| `cover-letter.test.ts` | **Keep** | Profile page, generate section, schema for communication profiles. |
| `cover-letter-quality.test.ts` | **Keep** | Revision client auth header, API routes, word-cap UI. |
...
| `resume-pdf-page-count.test.ts` | **Replace Later** | Page-count logic testable without reading route source. |
...
**Delete Now:** none — all grep tests currently protect real contracts or recent regressions (e.g. revision `Authorization` header).
