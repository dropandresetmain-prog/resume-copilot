...
> Archived pre-recovery manual checklist. Use [`../../TEST_CHECKLIST.md`](../../TEST_CHECKLIST.md) for the current canonical flow.

Run `npm run test`, `npm run lint`, and `npm run build` before manual QA.

Older milestone checklists were removed during repo cleanup. Completed phase docs live under [`docs/archive/`](../). Live Gemini tailoring comparison: [`PHASE0_MANUAL_QA.md`](PHASE0_MANUAL_QA.md).
