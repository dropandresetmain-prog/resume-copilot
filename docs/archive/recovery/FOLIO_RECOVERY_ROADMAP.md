...
| **M10b** | Output Editor redesign — implementation | Implement agreed design and close the independent review findings before merge | High |
| **M10c** | Output Editor + Vault quick fixes | Low-risk display/interaction fixes surfaced reviewing M10b output | Low |
| **M11** | Output Editor v2 — unified staging + batched apply + content/layout gate; Vault structured editing | Make editing deliberate/batched/consistent; close independent review findings before merge | High |
...
### M10c — Output Editor + Vault Quick Fixes

- **Objective:** Land the low-risk display/interaction fixes surfaced reviewing the M10b output, without touching engines or gate semantics.
- **Outcome (all complete):**
  1. **Vault tab order** — reordered to Work experience → Education → Skills → Additional (was Work → Skills → Education → Additional).
  2. **Vault Keyword Bank → tab** — removed the uncollapsed block under the vault-health card; Keyword Bank is now a 5th tab ("Keywords") alongside the others.
  3. **Output text view** — removed the redundant "Professional summary" heading (kept the Edit affordance, right-aligned).
  4. **Export & delivery** — Export PDF / Export DOCX / Re-approve now sit on one row.
  5. **PDF view declutter** — in PDF view the right controls column (Included experience, Custom revision, Shape next regeneration, diagnostics) is hidden; only the Layout sliders + preview show. The document column goes full width. Controls remain in Text view.
- **Stays mounted:** `OutputEditorPageClient`, `CareerVaultPageClient`.
- **Must not change:** export engine, gate semantics, generation engine, overlay/data wiring. (None changed.)
- **Tests:** `app-shell` (route-contract) + `application-package-ux` green; build green.
- **Model:** Sonnet. **Effort:** Low.
- **Dependencies:** M10b.

---

### M11 — Output Editor v2: Unified Staging + Batched Apply + Content/Layout Gate; Vault Structured Editing

- **Objective:** Replace the current mix of immediate-AI and immediate-content edits with a single **staged-changes** model applied via explicit batched AI actions, gated behind a content→layout→approve→export sequence. Extend structured overlay editing to all vault/resume sections.
- **Why here:** M10b correctly stacked the redesigned Output Editor, but several interactions still fire AI immediately (CL chips/tone), edits are not stage-and-review, and non-Work sections are read-only blobs. This milestone makes editing deliberate, batched, and consistent. **User decisions captured 2026-06-30 (below) — no separate design doc; this section is the spec.**
- **Approved design decisions (from user, 2026-06-30):**

  | Topic | Decision |
  |---|---|
  | **Bullet "Replace"** | User picks from **spine-ranked alternative bullets** for that role (the legacy force-bullet idea, ranked by `buildEvidenceSpine`), **then** the picked set is sent to AI **one-shot** to tailor to JD context. Not instant-swap, not blind-AI — pick-then-tailor. Reuses the additive `single_bullet` scope as the tailoring call. |
  | **Apply staged changes** | **Two separate buttons** (resume vs cover letter), not one combined call and not a new combined endpoint. **Resume button:** targeted revision of **only the changed/added bullets** — must NOT regenerate or re-touch the rest of the already-generated resume (use `single_bullet`-style targeted tailoring, never a full regenerate). **Cover letter button:** full CL regenerate that folds in all staged CL changes (tone + chips + evidence use/avoid + custom instruction) in one call. |
  | **Vault + Output section editing** | **Structured overlay** (same model as Work Experience: per-item add / edit / hide / revert in the `InventoryEdits` overlay) extended to **Education, Skills, Additional Experience**. Source resumes never mutated. The same structured editing flows into the Output text view for those sections (no more inline-only weak edit). |
  | **Staging persistence** | Staged changes (resume bucket + CL bucket) **persist while the user picks** across both Resume and Cover Letter views in the session, until applied or cleared. |
  | **State gate** | New **UI-level** sequence: stage changes → **Apply with AI** (resume and/or CL) → **Confirm content** (resume) → confirming content **unlocks Layout** (PDF sliders) → **Approve for export** (existing server one-page 422 gate) → **download**. The persisted approval/export gate engine is unchanged; "confirm content" and "layout locked until confirmed" are added as UI locks in front of it. |
  | **Custom instructions** | Add a **custom instruction** input to BOTH the resume staging bucket and the CL staging bucket (CL custom instruction is currently missing). |
  | **CL chips/tone** | Must be **staged, not immediate-AI** (today `applyRevision`/`handleSelectTone` call Gemini on click). Chips, tone, evidence use/avoid, and custom instruction all stage into the CL bucket and apply only on the CL "Apply" button. |

- **Stays mounted:** `OutputEditorPageClient`, `CareerVaultPageClient`.
- **References (read only):** legacy force-bullet add/remove behavior; existing `single_bullet` scope, `buildEvidenceSpine`, `InventoryEdits` overlay (Work Experience edit/hide/revert as the structured-edit template).
- **Backend boundary:** prefer reusing the existing `single_bullet` `revise-resume-scope` branch for targeted resume tailoring; CL apply reuses the existing CL regenerate path with staged `evidenceControls` + instruction. Only additive endpoint changes if strictly required; **no** new combined resume+CL endpoint.
- **Must not change:** export engine, page-count truth, filename scheme, generation/repair engine internals, model IDs, evidence spine; M4/M5a approval & export gate semantics (server one-page 422 hard gate; `resolveDraftStatusAfterContentEdit` invalidation; `areExportLayoutSettingsEqual`). The "confirm content / lock layout" step is a UI gate only — it must not weaken the server gate.
- **Tests:** extend `resume-draft-review`, `application-package-ux`, `inventory-edits` (structured editing for Education/Skills/Additional); keep `app-shell` route-contract green.
...
CONTEXT: Read docs/FOLIO_RECOVERY_ROADMAP.md and docs/OUTPUT_EDITOR_DESIGN.md in full before doing anything else. docs/OUTPUT_EDITOR_DESIGN.md is the source of truth for this milestone. M1–M9 and M10a are complete. MX (Career Vault overhaul) is parked — not a dependency.

IMPLEMENT EXACTLY THE AGREED DESIGN FROM M10a (docs/OUTPUT_EDITOR_DESIGN.md). Do not add features beyond what was agreed. Do not revisit design decisions — those are settled.

REPO: C:\Dev\AIAP\resume-copilot
BRANCH: folio-recovery. Confirm with `git branch --show-current`. Do NOT touch main (a4d17e3, production).

NON-NEGOTIABLE: OutputEditorPageClient stays mounted at /output/[draftId]. Never swap an active route to a legacy page client. No active page.tsx may import InventoryPageClient, RecordsPageClient, GeneratePageClient, ResumePreviewPageClient, or CoverLetterPreviewPageClient. The forbidden-remount rule is enforced by tests/suites/app-shell.test.ts — keep it green.

DESIGN: All UI must follow docs/DESIGN.md and docs/OUTPUT_EDITOR_DESIGN.md. Sentence case; no shadows (hairline borders + tonal surfaces); folio-* tokens; rounded-xl cards; rounded-lg controls.

PROTOCOL: Ask clarifying questions before writing any code. State what you plan to do and wait for confirmation.

AGREED DESIGN (full detail in docs/OUTPUT_EDITOR_DESIGN.md — summary so you do not re-derive it):
1. Fit summary = full-width banner directly under the header. Wire calculateFitScore(draft.content, draft.rationale) (deterministic, NO page-load AI) and pass BOTH rationale AND fitAssessment into buildPackageFitSummary — the current code passes only { rationale } so no numeric score ever shows. Surface score chip NN/100 + verdict + strengths line + gaps line. Remove the right-column fit-summary disclosure.
2. Dual view = segmented "Text | PDF" control in the document (left) column, one at a time. Text = RenderedResume (default, with selectable bullets). PDF = ResumePdfPreview built from buildExportResumeDocumentModel — available ON DEMAND, NO approval required (drop the current "PDF only after exportReady" coupling).
3. Selectable bullet blocks in Text view, per-bullet actions Edit / Replace / Remove. Edit + Remove reuse the existing StructuredResumeEditor mutators as immediate content edits (this RETIRES the whole-panel "Edit resume" toggle). Replace = single-bullet regeneration via a NEW approved single_bullet revision scope — add it to ResumeCustomRevisionScope and a new branch in POST /api/ai/revise-resume-scope alongside professional_summary and selected_role. This is ADDITIVE: existing scopes' behavior and the generation/repair engine internals must not change. Every content-mutating action runs the existing resolveDraftStatusAfterContentEdit invalidation. Keep lineLevelExcludedBulletKeys/lineLevelForcedBulletKeys OUT of the bullet menu — they live in the right-column "Shape next regeneration" disclosure (renamed from "Bullet controls"), labeled "affects regeneration, not the current document".
4. Layout sliders (body font, side margins, top margin, line spacing, section spacing) = Folio-native control strip shown ONLY in PDF view; slider → manualSettings → rebuild documentModel → live ResumePdfPreview re-render (mirror ResumePreviewPageClient wiring). Surface the auto-optimizer note. Layout invalidation already handled by areExportLayoutSettingsEqual — no new logic.
5. Export consolidation = ONE full-width "Export & delivery" card at the BOTTOM of the Resume tab (approval state, server one-page gate result + suggestedActions, layout_changed re-approve notice, Approve → Export PDF/DOCX gated on exportReady). REMOVE the duplicate topbar Export PDF/DOCX buttons. needs_review stays as a TOP banner near the fit summary.
6. REMOVE "Mark as sent" entirely (handleMarkSent, topbar CTA, markedSent state) — it duplicates Applications status edit (M6).
7. Right panel = controls only: experience inclusion (visible), custom AI revision (disclosure), shape-next-regeneration (line-level, disclosure), tailoring diagnostics (disclosure). Fit summary moved to top; per-bullet edit moved into the document.
8. CL tab secondary formats = collapsible "Other formats" section below the CL body, copy-paste blocks for email/LinkedIn/recruiter DM/WhatsApp read from coverLetter.rationale (precomputed strings, NO generation). Restyle the slate SecondaryCommunicationsPanel to Folio-native; hide when none present.

RESTYLE TO FOLIO-NATIVE (DESIGN.md): the ResumePdfPreview wrapper chrome (currently shadow-xl/slate-*; leave the iframe print HTML untouched — it is export truth), the ported layout sliders, and SecondaryCommunicationsPanel.

STAYS MOUNTED: OutputEditorPageClient.

MUST NOT CHANGE: export engine, page-count truth, filename scheme, generation/repair engine internals, model IDs, evidence spine; M4/M5a/M5b/M5c approval & export gate semantics (server one-page 422 hard gate remains export truth; resolveDraftStatusAfterContentEdit invalidation on content edit; areExportLayoutSettingsEqual layout invalidation). The ONLY permitted additive backend change is the new single_bullet revision scope + revise-resume-scope branch (item 3).

INDEPENDENT REVIEW: required before merge (same standard as M4). After implementation and verification, open a fresh Opus chat with the review brief + diff only — no implementation history.

CHECKS: npm run test, npm run lint, npm run build. Extend existing suites only — resume-draft-review, forced-bullet-regeneration, application-package-ux, app-shell route-contract; add single-bullet revision-scope coverage to the relevant revision suite. Update docs under /docs only.

KNOWN PRE-EXISTING RED (NOT introduced by you; do NOT fix unless explicitly scoped): resume-generation-validation.test.ts (3 fails); draft-inventory-safety.test.ts (2 fails); 1 lint error in ProfilePageClient.tsx; 1 TS error in application-records.test.ts. Report if they block verification but do not expand scope.
