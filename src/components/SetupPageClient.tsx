...
export function SetupPageClient() {
  ...
  function handleClearAll() {
    const confirmed = window.confirm(
      "Clear all uploaded resumes, parsing errors, and saved browser inventory?",
    );
    if (!confirmed) return;
    clearInventoryStorage();
      ...
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-6 px-4 py-8 lg:px-8">
        <header className="space-y-2">
          ...
          <p className="max-w-3xl text-base text-zinc-600">
            ...
            DOCX files are parsed in the browser. Original files are not stored.
            Parsed inventory can be saved in this browser or exported as JSON.
          </p>
        </header>
